...
static void mt7902s_card_reset(struct work_struct *work)
{
...
static DECLARE_WORK(sdio_reset_work, mt7902s_card_reset);
static int mt7902s_check_bus(struct mt76_dev *dev)
{
	struct mt76_sdio *sdio = &dev->sdio;
	int err;
...
static int mt7902s_host_reset(struct mt792x_dev *dev)
{
	struct mt76_dev *mdev = &dev->mt76;
	int err = -1;

	if (!atomic_read(&mdev->bus_hung))
		err = mt7902s_check_bus(&dev->mt76);
