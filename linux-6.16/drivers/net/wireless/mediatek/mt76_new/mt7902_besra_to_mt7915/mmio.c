...
enum {
	...
	mt7902_REG_WR,
	mt7902_REG_RMW,
	__mt7902_REG_OPS_MAX,
};

static u32 mt7902_reg_wr(struct mt7902_dev *dev, u32 addr, u32 mask, u32 val, u8 wr)
{
	u32 ret = 0;
...
static u32 mt7902_reg_map_l1(struct mt7902_dev *dev, u32 addr, u32 mask, u32 val, u8 wr)
{
	...
	ret = mt7902_reg_wr(dev, MT_HIF_REMAP_BASE_L1 + offset, mask, val, wr);
...
static u32 mt7902_reg_map_l2(struct mt7902_dev *dev, u32 addr, u32 mask, u32 val, u8 wr)
{
	...
	ret = mt7902_reg_wr(dev, MT_HIF_REMAP_BASE_L2 + offset, mask, val, wr);
...
static u32 __mt7902_reg_addr(struct mt7902_dev *dev, u32 addr, u32 mask, u32 val, u8 wr)
{
	int i;

	if (addr < 0x100000)
		return mt7902_reg_wr(dev, addr, mask, val, wr);
	...
	if ((addr >= MT_INFRA_BASE && addr < MT_WFSYS0_PHY_START) ||
		...
		return mt7902_reg_map_l1(dev, addr, mask, val, wr);
	...
	return mt7902_reg_map_l2(dev, addr, mask, val, wr);
}
...
static u32 mt7902_rr(struct mt76_dev *mdev, u32 offset)
{
	...
	return __mt7902_reg_addr(dev, offset, 0, 0, mt7902_REG_RR);
}
...
static void mt7902_wr(struct mt76_dev *mdev, u32 offset, u32 val)
{
	...
	__mt7902_reg_addr(dev, offset, 0, val, mt7902_REG_WR);
}
...
static u32 mt7902_rmw(struct mt76_dev *mdev, u32 offset, u32 mask, u32 val)
{
	...
	return __mt7902_reg_addr(dev, offset, mask, val, mt7902_REG_RMW);
}
...
{
	struct mt7902_dev *dev = from_tasklet(dev, t, irq_tasklet);
	u32 intr, intr1, mask;
				     ...
				     void __iomem *mem_base, u32 device_id)
{
	static const struct mt76_driver_ops drv_ops = {
		...
		.txwi_size = MT_TXD_SIZE + sizeof(struct mt7902_txp),
		...
		.tx_complete_skb = mt7902_tx_complete_skb,
	...
	tasklet_setup(&dev->irq_tasklet, mt7902_irq_tasklet);
