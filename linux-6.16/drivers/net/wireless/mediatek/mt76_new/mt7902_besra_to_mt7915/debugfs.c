...
static int mt7902_ser_trigger_set(void *data, u64 val)
{
	enum {
	...
	case SER_SET_RECOVER_L2:
		ret = mt7902_mcu_set_ser(dev, SER_ENABLE, BIT(val), 0);
		if (ret)
			return ret;

		return mt7902_mcu_set_ser(dev, SER_RECOVER, val, 0);
	default:
		break;
	}
...
mt7902_radar_trigger(void *data, u64 val)
{
	...
	return mt7902_mcu_rdd_cmd(dev, RDD_RADAR_EMULATE,
				 val, 0, 0);
}
...
static int mt7902_muru_stats_show(struct seq_file *file, void *data)
{
	...
	ret = mt7902_mcu_muru_debug_get(phy, &mu_stats);
	if (ret)
		goto exit;
...
mt7902_fw_debug_wa_set(void *data, u64 val)
{
	...
	ret = mt7902_mcu_fw_log_2_host(dev, MCU_FW_LOG_WA, dev->fw_debug_wa);
	if (ret)
		return ret;

	return mt7902_mcu_wa_cmd(dev, MCU_WA_PARAM_CMD(SET), MCU_WA_PARAM_PDMA_RX,
				 !!dev->fw_debug_wa, 0);
}
...
mt7902_fw_debug_bin_set(void *data, u64 val)
{
	...
	return mt7902_fw_debug_wm_set(dev, dev->fw_debug_wm);
}
...
static int
mt7902_rate_txpower_show(struct seq_file *file, void *data)
{
	...
	struct mt7902_phy *phy = file->private;
	s8 txpower[mt7902_SKU_RATE_NUM], *buf;
	int i;

	seq_printf(file, "\nBand %d\n", phy->band_idx);
	mt7902_mcu_get_txpower_sku(phy, txpower, sizeof(txpower));
	for (i = 0, buf = txpower; i < ARRAY_SIZE(mt7902_sku_group_len); i++) {
		u8 mcs_num = mt7902_sku_group_len[i];

		if (i >= SKU_VHT_BW20 && i <= SKU_VHT_BW160)
			mcs_num = 10;

		mt76_seq_puts_array(file, sku_group_name[i], buf, mcs_num);
		buf += mt7902_sku_group_len[i];
	}

	return 0;
}

DEFINE_SHOW_ATTRIBUTE(mt7902_rate_txpower);
...
mt7902_rf_regval_set(void *data, u64 val)
{
	...
	return mt7902_mcu_rf_regval(dev, dev->mt76.debugfs_reg, (u32 *)&val, true);
}
