...
static const struct mt7902_dfs_radar_spec etsi_radar_specs = {
...
static const struct mt7902_dfs_radar_spec fcc_radar_specs = {
	.pulse_th = { 110, -10, -80, 40, 5200, 128, 5200 },
	.radar_pattern = {
...
static const struct mt7902_dfs_radar_spec jp_radar_specs = {
	.pulse_th = { 110, -10, -80, 40, 5200, 128, 5200 },
	.radar_pattern = {
		[0] =  { 1, 0,  8,  32, 28, 0,  508, 3076,  13, 1,  1 },
		[1] =  { 1, 0, 12,  32, 28, 0,  140,  240,  17, 1,  1 },
...
static struct mt76_wcid *mt7902_rx_get_wcid(struct mt7902_dev *dev,
					    u16 idx, bool unicast)
{
	...
	sta = container_of(wcid, struct mt7902_sta, wcid);
	if (!sta->vif)
		return NULL;
...
static void
mt7902_mac_decode_he_radiotap_ru(struct mt76_rx_status *status,
...
static void
mt7902_mac_decode_he_mu_radiotap(struct sk_buff *skb, __le32 *rxv)
{
...
static void
mt7902_mac_decode_he_radiotap(struct sk_buff *skb, __le32 *rxv, u8 mode)
{
	struct mt76_rx_status *status = (struct mt76_rx_status *)skb->cb;
	...
	case MT_PHY_TYPE_HE_MU:
		...
		mt7902_mac_decode_he_radiotap_ru(status, he, rxv);
		mt7902_mac_decode_he_mu_radiotap(skb, rxv);
		break;
...
static int mt7902_reverse_frag0_hdr_trans(struct sk_buff *skb, u16 hdr_gap)
{
	struct mt76_rx_status *status = (struct mt76_rx_status *)skb->cb;
	struct ethhdr *eth_hdr = (struct ethhdr *)(skb->data + hdr_gap);
	struct mt7902_sta *msta = (struct mt7902_sta *)status->wcid;
...
static int
mt7902_mac_fill_rx_rate(struct mt7902_dev *dev,
...
static int
mt7902_mac_fill_rx(struct mt7902_dev *dev, struct sk_buff *skb)
{
	struct mt76_rx_status *status = (struct mt76_rx_status *)skb->cb;
	struct mt76_phy *mphy = &dev->mt76.phy;
	struct mt7902_phy *phy = &dev->phy;
	struct ieee80211_supported_band *sband;
	__le32 *rxd = (__le32 *)skb->data;
	__le32 *rxv = NULL;
	...
	mphy = mt76_dev_phy_by_band(&dev->mt76, band_idx);
	phy = mphy->priv;
	status->phy_idx = mt76_get_phy_id(mphy);
	...
	status->wcid = mt7902_rx_get_wcid(dev, idx, unicast);

	if (status->wcid) {
		struct mt7902_sta *msta;

		msta = container_of(status->wcid, struct mt7902_sta, wcid);
		spin_lock_bh(&dev->sta_poll_lock);
	...
	if (rxd1 & MT_RXD1_NORMAL_GROUP_3) {
		...
		ret = mt7902_mac_fill_rx_rate(dev, status, sband, rxv, &mode);
		if (ret < 0)
			return ret;
	}
	...
	if (hdr_trans && ieee80211_has_morefrags(fc)) {
		if (mt7902_reverse_frag0_hdr_trans(skb, hdr_gap))
			return -EINVAL;
		hdr_trans = false;
	} else {
		int pad_start = 0;
	...
	if (rxv && mode >= MT_PHY_TYPE_HE_SU && !(status->flag & RX_FLAG_8023))
		mt7902_mac_decode_he_radiotap(skb, rxv, mode);
...
static void
mt7902_mac_fill_rx_vector(struct mt7902_dev *dev, struct sk_buff *skb)
{
	...
	if (phy_idx == MT_EXT_PHY) {
		phy = mt7902_ext_phy(dev);
		if (!phy)
			goto out;
	}

	if (phy_idx == MT_TRI_PHY) {
		phy = mt7902_tri_phy(dev);
		if (!phy)
			goto out;
	}
...
static void
mt7902_mac_write_txwi_tm(struct mt7902_phy *phy, __le32 *txwi,
			 struct sk_buff *skb)
{
#ifdef CONFIG_NL80211_TESTMODE
...
static void
mt7902_mac_write_txwi_8023(struct mt7902_dev *dev, __le32 *txwi,
...
static void
mt7902_mac_write_txwi_80211(struct mt7902_dev *dev, __le32 *txwi,
			    ...
			    bool *mcast)
{
	...
	*mcast = is_multicast_ether_addr(hdr->addr1);
	...
	txwi[3] |= FIELD_PREP(MT_TXD3_BCM, *mcast);
...
static u16
mt7902_mac_tx_rate_val(struct mt76_phy *mphy, struct ieee80211_vif *vif,
			   ...
			   struct ieee80211_key_conf *key, bool beacon)
{
	...
	mphy = mt76_dev_phy(&dev->mt76, phy_idx);
	...
	if (beacon) {
		...
		p_fmt = MT_TX_TYPE_CT;
		q_idx = wmm_idx * mt7902_MAX_WMM_SETS +
	...
	/* Todo: check MT_TXD6_DIS_MAT for MLD to Link Address Translation */
	/* val = MT_TXD6_DIS_MAT | MT_TXD6_VTA | */
	...
	if (is_8023)
		mt7902_mac_write_txwi_8023(dev, txwi, skb, wcid);
	else
		mt7902_mac_write_txwi_80211(dev, txwi, skb, key, &mcast);

	if (txwi[1] & cpu_to_le32(MT_TXD1_FIXED_RATE)) {
		u16 rate = mt7902_mac_tx_rate_val(mphy, vif, beacon, mcast);

		/* hardware won't add HTC for mgmt/ctrl frame */
		/* txwi[2] |= cpu_to_le32(MT_TXD2_HTC_VLD); */

		val = FIELD_PREP(MT_TXD6_BW, 8) |
		      FIELD_PREP(MT_TXD6_TX_RATE, 1);
		txwi[6] |= cpu_to_le32(val);
		//txwi[3] |= cpu_to_le32(MT_TXD3_BA_DISABLE);
	}
			  ...
			  struct mt76_tx_info *tx_info)
{
	...
	if (sta) {
		...
		msta = (struct mt7902_sta *)sta->drv_priv;
	...
	if (!is_8023 || pid >= MT_PACKET_ID_FIRST)
		mt7902_mac_write_txwi(dev, txwi_ptr, tx_info->skb, wcid, pid,
				     key, false);

	txp = (struct mt7902_txp *)(txwi + MT_TXD_SIZE);
	for (i = 0; i < nbuf; i++) {
...
static void
mt7902_tx_check_aggr(struct ieee80211_sta *sta, __le32 *txwi)
{
	struct mt7902_sta *msta;
	u16 fc, tid;
	u32 val;
...
static void
mt7902_txp_skb_unmap(struct mt76_dev *dev, struct mt76_txwi_cache *t)
{
	struct mt7902_txp *txp;
	int i;

	txp = mt7902_txwi_to_txp(dev, t);
...
static void
mt7902_txwi_free(struct mt7902_dev *dev, struct mt76_txwi_cache *t,
...
static void
mt7902_mac_tx_free(struct mt7902_dev *dev, void *data, int len)
{
	struct mt7902_tx_free *free = (struct mt7902_tx_free *)data;
	struct mt76_dev *mdev = &dev->mt76;
	...
	if (mphy_tri) {
		mt76_queue_tx_cleanup(dev, mphy_tri->q_tx[MT_TXQ_PSD], false);
	...
	for (cur_info = &free->info[0]; count < total; cur_info++) {
		...
		if (info & MT_TX_FREE_PAIR) {
			...
			msta = container_of(wcid, struct mt7902_sta, wcid);
			spin_lock_bh(&dev->sta_poll_lock);
			if (list_empty(&msta->poll_list))
				list_add_tail(&msta->poll_list, &dev->sta_poll_list);
			spin_unlock_bh(&dev->sta_poll_lock);
		...
		for (i = 0; i < 1 + v3; i++) {
			...
			mt7902_txwi_free(dev, txwi, sta, &free_list);
		}
	}
...
static bool
mt7902_mac_add_txs_skb(struct mt7902_dev *dev, struct mt76_wcid *wcid, int pid,
...
static void mt7902_mac_add_txs(struct mt7902_dev *dev, void *data)
{
	struct mt7902_sta *msta = NULL;
	struct mt76_wcid *wcid;
	...
	mt7902_mac_add_txs_skb(dev, wcid, pid, txs_data, &msta->stats);
...
bool mt7902_rx_check(struct mt76_dev *mdev, enum mt76_rxq_id q, void *data, int len)
{
	...
	case PKT_TYPE_TXRX_NOTIFY:
		mt7902_mac_tx_free(dev, data, len);
		return false;
	case PKT_TYPE_TXS:
	...
	case PKT_TYPE_RX_FW_MONITOR:
		mt7902_debugfs_rx_fw_monitor(dev, data, len);
		return false;
	default:
		return true;
	}
			 ...
			 struct sk_buff *skb)
{
	...
	case PKT_TYPE_RX_EVENT:
		...
		break;
	case PKT_TYPE_TXRXV:
		mt7902_mac_fill_rx_vector(dev, skb);
		break;
	case PKT_TYPE_TXS:
		for (rxd += 4; rxd + 8 <= end; rxd += 8)
			mt7902_mac_add_txs(dev, rxd);
		...
		break;
	case PKT_TYPE_RX_FW_MONITOR:
		mt7902_debugfs_rx_fw_monitor(dev, skb->data, skb->len);
		dev_kfree_skb(skb);
		break;
...
void mt7902_tx_complete_skb(struct mt76_dev *mdev, struct mt76_queue_entry *e)
{
	...
	if (e->skb == DMA_DUMMY_DATA) {
		...
		txp = mt7902_txwi_to_txp(mdev, e->txwi);
...
void mt7902_mac_reset_counters(struct mt7902_phy *phy)
{
	...
	mt7902_mcu_get_chan_mib_info(phy, true);
}
...
void mt7902_mac_set_timing(struct mt7902_phy *phy)
{
	...
	struct mt7902_dev *dev = phy->dev;
	struct mt7902_phy *ext_phy = mt7902_ext_phy(dev);
	struct mt7902_phy *tri_phy = mt7902_tri_phy(dev);
	u32 val, reg_offset;
...
void mt7902_mac_enable_nf(struct mt7902_dev *dev, u8 band)
{
	...
	mt76_set(dev, MT_WF_PHY_RX_CTRL1(band),
		 FIELD_PREP(MT_WF_PHY_RX_CTRL1_IPI_EN, 0x5));
}

static u8
mt7902_phy_get_nf(struct mt7902_phy *phy, int idx)
{
	...
	for (nss = 0; nss < hweight8(phy->mt76->chainmask); nss++) {
		u32 reg = MT_WF_IRPI_NSS(idx, nss);
...
{
	struct mt7902_phy *phy = (struct mt7902_phy *)mphy->priv;
	struct mt76_channel_state *state = mphy->chan_state;
	int nf;

	mt7902_mcu_get_chan_mib_info(phy, false);

	nf = mt7902_phy_get_nf(phy, phy->band_idx);
	if (!phy->noise)
		phy->noise = nf << 4;
	else if (nf)
...
static bool
mt7902_wait_reset_state(struct mt7902_dev *dev, u32 state)
{
	bool ret;

	ret = wait_event_timeout(dev->reset_wait,
				 (READ_ONCE(dev->reset_state) & state),
				 mt7902_RESET_TIMEOUT);
...
static void
mt7902_update_vif_beacon(void *priv, u8 *mac, struct ieee80211_vif *vif)
{
	...
	case NL80211_IFTYPE_AP:
		mt7902_mcu_add_beacon(hw, vif, vif->bss_conf.enable_beacon);
		break;
	default:
		break;
	}
...
static void
mt7902_update_beacons(struct mt7902_dev *dev)
{
	ieee80211_iterate_active_interfaces(dev->mt76.hw,
		IEEE80211_IFACE_ITER_RESUME_ALL,
		mt7902_update_vif_beacon, dev->mt76.hw);
	...
	ieee80211_iterate_active_interfaces(dev->mt76.phy2->hw,
		IEEE80211_IFACE_ITER_RESUME_ALL,
		mt7902_update_vif_beacon, dev->mt76.phy2->hw);
	...
	ieee80211_iterate_active_interfaces(dev->mt76.phy3->hw,
		IEEE80211_IFACE_ITER_RESUME_ALL,
		mt7902_update_vif_beacon, dev->mt76.phy3->hw);
}

static void
mt7902_dma_reset(struct mt7902_dev *dev)
{
	...
	for (i = 0; i < __MT_TXQ_MAX; i++) {
		...
		if (mphy_tri)
			mt76_queue_tx_cleanup(dev, mphy_tri->q_tx[i], true);
	}
...
void mt7902_tx_token_put(struct mt7902_dev *dev)
{
	...
	idr_for_each_entry(&dev->mt76.token, txwi, id) {
		mt7902_txwi_free(dev, txwi, NULL, NULL);
...
{
	struct mt7902_phy *phy2, *phy3;
	struct mt76_phy *ext_phy, *tri_phy;
	struct mt7902_dev *dev;

	dev = container_of(work, struct mt7902_dev, reset_work);
	ext_phy = dev->mt76.phy2;
	phy2 = ext_phy ? ext_phy->priv : NULL;

	tri_phy = dev->mt76.phy3;
	phy3 = tri_phy ? tri_phy->priv : NULL;
	...
	if (tri_phy)
		ieee80211_stop_queues(tri_phy->hw);
	...
	if (mt7902_wait_reset_state(dev, MT_MCU_CMD_RESET_DONE)) {
		mt7902_dma_reset(dev);

		mt7902_tx_token_put(dev);
		idr_init(&dev->mt76.token);

		mt76_wr(dev, MT_MCU_INT_EVENT, MT_MCU_INT_EVENT_DMA_INIT);
		mt7902_wait_reset_state(dev, MT_MCU_CMD_RECOVERY_DONE);
	}
	...
	mt7902_wait_reset_state(dev, MT_MCU_CMD_NORMAL_STATE);
	...
	if (tri_phy)
		ieee80211_wake_queues(tri_phy->hw);

	mutex_unlock(&dev->mt76.mutex);

	mt7902_update_beacons(dev);

	ieee80211_queue_delayed_work(mt76_hw(dev), &dev->mphy.mac_work,
				     mt7902_WATCHDOG_TIME);
	if (phy2)
...
void mt7902_mac_update_stats(struct mt7902_phy *phy)
{
	...
	mib->tx_pkt_ibf_cnt += FIELD_GET(MT_MIB_SDR32_TX_PKT_IBF_CNT_MASK, cnt);
...
{
	struct mt7902_dev *dev = container_of(work, struct mt7902_dev, rc_work);
	struct ieee80211_sta *sta;
	struct ieee80211_vif *vif;
	struct mt7902_sta *msta;
	u32 changed;
	...
	while (!list_empty(&list)) {
		msta = list_first_entry(&list, struct mt7902_sta, rc_list);
		...
		if (changed & (IEEE80211_RC_SUPP_RATES_CHANGED |
			...
			mt7902_mcu_add_rate_ctrl(dev, vif, sta, true);

		if (changed & IEEE80211_RC_SMPS_CHANGED)
			mt7902_mcu_add_smps(dev, vif, sta);
...
static void mt7902_dfs_stop_radar_detector(struct mt7902_phy *phy)
{
	struct mt7902_dev *dev = phy->dev;

	if (phy->rdd_state & BIT(0))
		mt7902_mcu_rdd_cmd(dev, RDD_STOP, 0,
				  MT_RX_SEL0, 0);
	if (phy->rdd_state & BIT(1))
		mt7902_mcu_rdd_cmd(dev, RDD_STOP, 1,
				  MT_RX_SEL0, 0);
}

static int mt7902_dfs_start_rdd(struct mt7902_dev *dev, int chain)
{
	int err, region;
	...
	err = mt7902_mcu_rdd_cmd(dev, RDD_START, chain,
				MT_RX_SEL0, region);
	if (err < 0)
		return err;

	return mt7902_mcu_rdd_cmd(dev, RDD_DET_MODE, chain,
				 MT_RX_SEL0, 1);
}

static int mt7902_dfs_start_radar_detector(struct mt7902_phy *phy)
{
	...
	/* start CAC */
	err = mt7902_mcu_rdd_cmd(dev, RDD_CAC_START, phy->band_idx,
				MT_RX_SEL0, 0);
	if (err < 0)
		return err;

	err = mt7902_dfs_start_rdd(dev, phy->band_idx);
	if (err < 0)
		return err;
	...
	if (chandef->width == NL80211_CHAN_WIDTH_160 ||
		...
		err = mt7902_dfs_start_rdd(dev, 1);
		if (err < 0)
			return err;
...
static int
mt7902_dfs_init_radar_specs(struct mt7902_phy *phy)
{
	const struct mt7902_dfs_radar_spec *radar_specs;
	struct mt7902_dev *dev = phy->dev;
	int err, i;
	...
	case NL80211_DFS_FCC:
		...
		err = mt7902_mcu_set_fcc5_lpn(dev, 8);
		if (err < 0)
			return err;
		break;
	case NL80211_DFS_ETSI:
	...
	for (i = 0; i < ARRAY_SIZE(radar_specs->radar_pattern); i++) {
		err = mt7902_mcu_set_radar_th(dev, i,
					      &radar_specs->radar_pattern[i]);
		if (err < 0)
			return err;
	}

	return mt7902_mcu_set_pulse_th(dev, &radar_specs->pulse_th);
}
...
int mt7902_dfs_init_radar_detector(struct mt7902_phy *phy)
{
	...
	if (prev_state == MT_DFS_STATE_UNKNOWN)
		mt7902_dfs_stop_radar_detector(phy);

	if (dfs_state == MT_DFS_STATE_DISABLED)
		goto stop;

	if (prev_state <= MT_DFS_STATE_DISABLED) {
		err = mt7902_dfs_init_radar_specs(phy);
		if (err < 0)
			return err;

		err = mt7902_dfs_start_radar_detector(phy);
		if (err < 0)
			return err;
	...
	err = mt7902_mcu_rdd_cmd(dev, RDD_CAC_END,
...
stop:
	err = mt7902_mcu_rdd_cmd(dev, RDD_NORMAL_START,
				phy->band_idx, MT_RX_SEL0, 0);
	if (err < 0)
		return err;
...
static int
mt7902_mac_twt_duration_align(int duration)
{
	return duration << 8;
}

static u64
mt7902_mac_twt_sched_list_add(struct mt7902_dev *dev,
			      struct mt7902_twt_flow *flow)
{
	struct mt7902_twt_flow *iter, *iter_next;
	u32 duration = flow->duration << 8;
	u64 start_tsf;

	iter = list_first_entry_or_null(&dev->twt_list,
					struct mt7902_twt_flow, list);
	...
	list_for_each_entry_safe(iter, iter_next, &dev->twt_list, list) {
		start_tsf = iter->start_tsf +
			    mt7902_mac_twt_duration_align(iter->duration);
...
static int mt7902_mac_check_twt_req(struct ieee80211_twt_setup *twt)
{
			      ...
			      struct ieee80211_twt_setup *twt)
{
	...
	if (mt7902_mac_check_twt_req(twt))
		goto out;

	mutex_lock(&dev->mt76.mutex);

	if (dev->twt.n_agrt == mt7902_MAX_TWT_AGRT)
		goto unlock;
	...
	if (sta_setup_cmd == TWT_SETUP_CMD_REQUEST ||
		...
		flow->sched = true;
		flow->start_tsf = mt7902_mac_twt_sched_list_add(dev, flow);
		curr_tsf = __mt7902_get_tsf(hw, msta->vif);
	...
	if (mt7902_mcu_twt_agrt_update(dev, msta->vif, flow, MCU_TWT_AGRT_ADD))
		goto unlock;
				  ...
				  u8 flowid)
{
	...
	flow = &msta->twt.flow[flowid];
	if (mt7902_mcu_twt_agrt_update(dev, msta->vif, flow,
				       MCU_TWT_AGRT_DELETE))
		return;
