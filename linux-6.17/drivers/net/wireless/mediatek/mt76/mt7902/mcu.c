			  ...
			  struct sk_buff *skb, int seq)
{
	...
	struct mt7902_mcu_uni_event *event;
	...
	rxd = (struct mt7902_mcu_rxd *)skb->data;
	if (seq != rxd->seq)
		return -EAGAIN;
	...
	if (cmd == MCU_CMD(PATCH_SEM_CONTROL)) {
		...
		event = (struct mt7902_mcu_uni_event *)skb->data;
		ret = le32_to_cpu(event->status);
		/* skip invalid event */
		if (mcu_cmd != event->cid)
			ret = -EAGAIN;
	} else {
		skb_pull(skb, sizeof(struct mt7902_mcu_rxd));
	}
...
mt7902_mcu_rx_thermal_notify(struct mt7902_dev *dev, struct sk_buff *skb)
{
	...
	phy = (struct mt7902_phy *)mphy->priv;
...
#define UNI_EVENT_FW_LOG_FORMAT 0
	struct mt7902_mcu_rxd *rxd = (struct mt7902_mcu_rxd *)skb->data;
...
static void
mt7902_mcu_ie_countdown(struct mt7902_dev *dev, struct sk_buff *skb)
{
...
static void
mt7902_mcu_uni_rx_unsolicited_event(struct mt7902_dev *dev, struct sk_buff *skb)
{
	struct mt7902_mcu_rxd *rxd = (struct mt7902_mcu_rxd *)skb->data;
	...
	case MCU_UNI_EVENT_IE_COUNTDOWN:
		mt7902_mcu_ie_countdown(dev, skb);
		break;
	default:
		break;
	}
			...
			struct mt7902_phy *phy)
{
	...
	tlv = mt7902_mcu_add_uni_tlv(skb, UNI_BSS_INFO_RLM, sizeof(*ch));
		      ...
		      struct mt7902_phy *phy)
{
	...
	tlv = mt7902_mcu_add_uni_tlv(skb, UNI_BSS_INFO_RA, sizeof(*ra));
...
mt7902_mcu_bss_bmc_tlv(struct sk_buff *skb, struct mt7902_phy *phy)
{
	...
	tlv = mt7902_mcu_add_uni_tlv(skb, UNI_BSS_INFO_RATE, sizeof(*bmc));
...
mt7902_mcu_bss_txcmd_tlv(struct sk_buff *skb, bool en)
{
	...
	tlv = mt7902_mcu_add_uni_tlv(skb, UNI_BSS_INFO_TXCMD, sizeof(*txcmd));
...
mt7902_mcu_bss_mld_tlv(struct sk_buff *skb)
{
	...
	tlv = mt7902_mcu_add_uni_tlv(skb, UNI_BSS_INFO_MLD, sizeof(*mld));
...
mt7902_mcu_bss_sec_tlv(struct sk_buff *skb, struct ieee80211_vif *vif)
{
	...
	tlv = mt7902_mcu_add_uni_tlv(skb, UNI_BSS_INFO_SEC, sizeof(*sec));
			...
			bool enable)
{
	...
	tlv = mt7902_mcu_add_uni_tlv(skb, UNI_BSS_INFO_BASIC, sizeof(*bss));
			    ...
			    struct ieee80211_vif *vif, int enable)
{
	...
	skb = __mt7902_mcu_alloc_bss_req(&dev->mt76, &mvif->mt76,
...
int mt7902_mcu_sta_ba(struct mt76_dev *dev, struct mt76_vif *mvif,
			 ...
			 bool enable)
{
	...
	return mt7902_mcu_sta_ba(&dev->mt76, &mvif->mt76, params,
				enable, true);
}
...
static void
mt7902_mcu_sta_phy_tlv(struct mt7902_dev *dev, struct sk_buff *skb,
		      struct ieee80211_vif *vif, struct ieee80211_sta *sta)
{
	struct mt7902_vif *mvif = (struct mt7902_vif *)vif->drv_priv;
				   ...
				   void *data, u32 field)
{
	...
	case RATE_PARAM_MMPS_UPDATE:
		ra->mmps_mode = mt7902_mcu_get_mmps_mode(sta->smps_mode);
		break;
	default:
		break;
	}
...
static int
mt7902_mcu_sta_key_tlv(struct mt76_wcid *wcid,
		      ...
		      struct mt76_wcid *wcid, enum set_key_cmd cmd)
{
	...
	ret = mt7902_mcu_sta_key_tlv(wcid, sta_key_conf, skb, key, cmd);
	if (ret)
		return ret;
			 ...
			 struct ieee80211_mutable_offsets *offs)
{
	...
	tlv = mt7902_mcu_add_uni_tlv(rskb, tag, sizeof(*info));
		       ...
		       struct ieee80211_mutable_offsets *offs)
{
	...
	mt7902_mac_write_txwi(dev, (__le32 *)buf, skb, wcid, 0, NULL,
			     true);
	memcpy(buf + MT_TXD_SIZE, skb->data, skb->len);
}

static void
mt7902_mcu_beacon_check_caps(struct mt7902_phy *phy, struct ieee80211_vif *vif,
			     struct sk_buff *skb)
{
	struct mt7902_vif *mvif = (struct mt7902_vif *)vif->drv_priv;
	struct mt7902_vif_cap *vc = &mvif->cap;
...
int mt7902_mcu_muru_debug_get(struct mt7902_phy *phy, void *ms)
{
	...
	struct mt7902_mcu_muru_stats *mu_stats =
				(struct mt7902_mcu_muru_stats *)ms;
	int ret;
	...
	memcpy(mu_stats, skb->data, sizeof(struct mt7902_mcu_muru_stats));
	dev_kfree_skb(skb);
...
int mt7902_mcu_set_hdr_trans(struct mt7902_dev *dev, bool hdr_trans)
{
	...
	struct sk_buff *skb;
	struct tlv *tlv;
	int len = mt7902_HDR_TRANS_MAX_SIZE + sizeof(hdr);
	int ret;
	...
	tlv = mt7902_mcu_add_uni_tlv(skb, UNI_HDR_TRANS_EN, sizeof(*req_en));
	req_en = (struct hdr_trans_en *)tlv;
	req_en->enable = hdr_trans;

	tlv = mt7902_mcu_add_uni_tlv(skb, UNI_HDR_TRANS_VLAN,
				    sizeof(struct hdr_trans_vlan));

	if (hdr_trans) {
		tlv = mt7902_mcu_add_uni_tlv(skb, UNI_HDR_TRANS_BLACKLIST,
...
int mt7902_mcu_set_tx(struct mt7902_dev *dev, struct ieee80211_vif *vif)
{
	...
	for (ac = 0; ac < IEEE80211_NUM_ACS; ac++) {
		...
		tlv = mt7902_mcu_add_uni_tlv(skb, MCU_EDCA_AC_PARAM, sizeof(*e));

		e = (struct edca *)tlv;
		e->set = WMM_PARAM_SET;
		e->queue = ac + mvif->mt76.wmm_idx * mt7902_MAX_WMM_SETS;
		e->aifs = q->aifs;
				     ...
				     struct cfg80211_chan_def *chandef)
{
	...
	if (!chandef) { /* disable offchain */
		err = mt7902_mcu_rdd_cmd(dev, RDD_STOP, MT_RX_SEL2,
					0, 0);
		if (err)
			return err;
	...
	return mt7902_mcu_rdd_cmd(dev, RDD_START, MT_RX_SEL2,
				 0, region);
}
...
#define PER_PAGE_SIZE		0x400
	...
	u16 eeprom_size = mt7902_EEPROM_SIZE;
...
int mt7902_mcu_get_chan_mib_info(struct mt7902_phy *phy, bool chan_switch)
{
	...
	res = (struct mt7902_mcu_mib *)(skb->data);
...
int mt7902_mcu_set_txpower_sku(struct mt7902_phy *phy)
{
	...
	struct mt7902_sku_val {
		u8 format_id;
		u8 limit_type;
		u8 band;
		s8 val[mt7902_SKU_RATE_NUM];
	} __packed req = {
		.format_id = 4,
		.band = phy->band_idx,
	};
...
#define ENTER_PM_STATE	1
	...
	tlv = mt7902_mcu_add_uni_tlv(skb, UNI_BSS_INFO_PS, sizeof(*ps));
