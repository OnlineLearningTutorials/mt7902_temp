...
int mt7902_mcu_regval(struct mt792x_dev *dev, u32 regidx, u32 *val, bool set)
{
...
EXPORT_SYMBOL_GPL(mt7902_mcu_regval);

int mt7902_mcu_update_arp_filter(struct mt76_dev *dev,
				 struct mt76_vif *vif,
...
#ifdef CONFIG_PM
static int
mt7902_connac_mcu_set_wow_ctrl(struct mt76_phy *phy, struct ieee80211_vif *vif,
			       bool suspend, struct cfg80211_wowlan *wowlan)
{
	...
	if (wowlan->nd_config) {
		mt7902_mcu_sched_scan_req(phy, vif, wowlan->nd_config);
		req.wow_ctrl_tlv.trigger |= UNI_WOW_DETECT_TYPE_SCH_SCAN_HIT;
		mt7902_mcu_sched_scan_enable(phy, vif, suspend);
	}
...
static int
mt7902_mcu_set_wow_pattern(struct mt76_dev *dev,
			   struct ieee80211_vif *vif,
			   u8 index, bool enable,
			   struct cfg80211_pkt_pattern *pattern)
{
	struct mt76_vif *mvif = (struct mt76_vif *)vif->drv_priv;
	struct mt7902_wow_pattern_tlv *tlv;
	struct sk_buff *skb;
	struct {
		u8 bss_idx;
		u8 pad[3];
	...
	tlv = (struct mt7902_wow_pattern_tlv *)skb_put(skb, sizeof(*tlv));
				 ...
				 struct ieee80211_vif *vif)
{
	...
	for (i = 0; i < wowlan->n_patterns; i++)
		mt7902_mcu_set_wow_pattern(phy->dev, vif, i, suspend,
					   &wowlan->patterns[i]);
	mt7902_connac_mcu_set_wow_ctrl(phy, vif, suspend, wowlan);
}
...
{
	struct mt76_vif *mvif = (struct mt76_vif *)vif->drv_priv;
	struct mt7902_uni_beacon_loss_event *event = priv;
...
{
	struct mt7902_uni_beacon_loss_event *event;
	struct mt76_phy *mphy = &dev->mt76.phy;

	skb_pull(skb, sizeof(struct mt7902_mcu_rxd));
	event = (struct mt7902_uni_beacon_loss_event *)skb->data;
...
static void
mt7902_mcu_roc_iter(void *priv, u8 *mac, struct ieee80211_vif *vif)
{
	struct mt76_vif *mvif = (struct mt76_vif *)vif->drv_priv;
	struct mt7902_roc_grant_tlv *grant = priv;
...
mt7902_mcu_uni_roc_event(struct mt792x_dev *dev, struct sk_buff *skb)
{
	...
	else if (grant->reqtype == MT7902_ROC_REQ_JOIN)
		ieee80211_iterate_active_interfaces_atomic(hw,
						IEEE80211_IFACE_ITER_RESUME_ALL,
						mt7902_mcu_roc_iter, grant);
...
#define UNI_EVENT_TX_DONE_RAW 1
	...
	skb_pull(skb, sizeof(struct mt7902_mcu_rxd) + 4);
	...
	while (tlv_len > 0 && le16_to_cpu(tlv->len) <= tlv_len) {
		...
		case UNI_EVENT_TX_DONE_RAW:
			txs = (struct mt7902_mcu_txs_event *)tlv->data;
			mt7902_mac_add_txs(dev, txs->data);
			break;
		default:
			break;
		}
...
static void
mt7902_mcu_uni_debug_msg_event(struct mt792x_dev *dev, struct sk_buff *skb)
{
	struct mt7902_uni_debug_msg {
		__le16 tag;
		__le16 len;
		u8 fmt;
		u8 rsv[3];
	...
	skb_pull(skb, sizeof(struct mt7902_mcu_rxd) + 4);
	hdr = (struct mt7902_uni_debug_msg *)skb->data;

	if (hdr->id == 0x28) {
		skb_pull(skb, offsetof(struct mt7902_uni_debug_msg, id));
				    ...
				    struct sk_buff *skb)
{
	...
	case MCU_UNI_EVENT_FW_LOG_2_HOST:
		mt7902_mcu_uni_debug_msg_event(dev, skb);
		break;
...
static int
mt7902_mcu_sta_ba(struct mt76_dev *dev, struct mt76_vif *mvif,
...
mt7902_mcu_parse_phy_cap(struct mt792x_dev *dev, char *data)
{
	...
	cap = (struct mt7902_mcu_phy_cap *)data;
...
static int
mt7902_mcu_get_nic_capability(struct mt792x_dev *dev)
{
	struct mt76_phy *mphy = &dev->mt76.phy;
	struct {
		u8 _rsv[4];
	...
	for (i = 0; i < le16_to_cpu(hdr->n_element); i++) {
		...
		case MT_NIC_CAP_MAC_ADDR:
			...
			break;
		case MT_NIC_CAP_PHY:
			mt7902_mcu_parse_phy_cap(dev, tlv->data);
			break;
		default:
			break;
		}
...
int mt7902_mcu_chip_config(struct mt792x_dev *dev, const char *cmd)
{
...
int mt7902_mcu_set_deep_sleep(struct mt792x_dev *dev, bool enable)
{
	char cmd[16];

	snprintf(cmd, sizeof(cmd), "KeepFullPwr %d", !enable);

	return mt7902_mcu_chip_config(dev, cmd);
}
EXPORT_SYMBOL_GPL(mt7902_mcu_set_deep_sleep);
...
int mt7902_run_firmware(struct mt792x_dev *dev)
{
	...
	err = mt7902_mcu_get_nic_capability(dev);
	if (err)
		return err;
...
static void
mt7902_mcu_sta_hdr_trans_tlv(struct sk_buff *skb,
			     struct ieee80211_vif *vif,
...
int mt7902_mcu_wtbl_update_hdr_trans(struct mt792x_dev *dev,
				     struct ieee80211_vif *vif,
				     struct ieee80211_sta *sta)
{
	struct mt792x_vif *mvif = (struct mt792x_vif *)vif->drv_priv;
	...
	skb = __mt76_connac_mcu_alloc_sta_req(&dev->mt76, &mvif->mt76,
					      &msta->wcid,
					      MT7902_STA_UPDATE_MAX_SIZE);
	if (IS_ERR(skb))
		return PTR_ERR(skb);

	/* starec hdr trans */
	mt7902_mcu_sta_hdr_trans_tlv(skb, vif, sta);
		       ...
		       enum set_key_cmd cmd)
{
	...
	if (cmd == SET_KEY) {
		...
		sec->add = 1;
		cipher = mt7902_mcu_get_cipher(key->cipher);
...
int mt7902_mcu_set_chan_info(struct mt792x_phy *phy, u16 tag)
{
...
static void
mt7902_mcu_sta_eht_tlv(struct sk_buff *skb, struct ieee80211_sta *sta)
{
...
static void
mt7902_mcu_sta_amsdu_tlv(struct sk_buff *skb,
...
static void
mt7902_mcu_sta_phy_tlv(struct sk_buff *skb,
		       struct ieee80211_vif *vif, struct ieee80211_sta *sta)
{
	struct mt792x_vif *mvif = (struct mt792x_vif *)vif->drv_priv;
...
static void
mt7902_mcu_sta_state_v2_tlv(struct mt76_phy *mphy, struct sk_buff *skb,
			    struct ieee80211_sta *sta,
			    struct ieee80211_vif *vif,
...
static void
mt7902_mcu_sta_rate_ctrl_tlv(struct sk_buff *skb,
			     struct ieee80211_vif *vif, struct ieee80211_sta *sta)
{
	struct mt792x_vif *mvif = (struct mt792x_vif *)vif->drv_priv;
	struct cfg80211_chan_def *chandef = &mvif->mt76.ctx->def;
...
static void
mt7902_mcu_sta_mld_tlv(struct sk_buff *skb,
		       struct ieee80211_vif *vif, struct ieee80211_sta *sta)
{
...
static int
mt7902_mcu_sta_cmd(struct mt76_phy *phy,
		   struct mt76_sta_cmd_info *info)
{
	...
	if (info->sta && info->enable) {
		mt7902_mcu_sta_phy_tlv(skb, info->vif, info->sta);
		mt7902_mcu_sta_ht_tlv(skb, info->sta);
		mt7902_mcu_sta_vht_tlv(skb, info->sta);
		mt76_connac_mcu_sta_uapsd(skb, info->vif, info->sta);
		mt7902_mcu_sta_amsdu_tlv(skb, info->vif, info->sta);
		mt7902_mcu_sta_he_tlv(skb, info->sta);
		mt7902_mcu_sta_he_6g_tlv(skb, info->sta);
		mt7902_mcu_sta_eht_tlv(skb, info->sta);
		mt7902_mcu_sta_rate_ctrl_tlv(skb, info->vif, info->sta);
		mt7902_mcu_sta_state_v2_tlv(phy, skb, info->sta,
					    info->vif, info->rcpi,
					    info->state);
		mt7902_mcu_sta_mld_tlv(skb, info->vif, info->sta);
	}

	if (info->enable)
		mt7902_mcu_sta_hdr_trans_tlv(skb, info->vif, info->sta);
			  ...
			  enum mt76_sta_info_state state)
{
	...
	return mt7902_mcu_sta_cmd(&dev->mphy, &info);
}
...
#define MT7902_FIF_BIT_SET		BIT(0)
	...
	if (enable) {
		...
		return mt7902_mcu_set_rxfilter(dev, 0,
					       MT7902_FIF_BIT_SET,
...
int mt7902_get_txpwr_info(struct mt792x_dev *dev, u8 band_idx, struct mt7902_txpwr *txpwr)
{
...
int mt7902_mcu_set_chctx(struct mt76_phy *phy, struct mt76_vif *mvif,
			 struct ieee80211_chanctx_conf *ctx)
{
...
static u8
mt7902_get_phy_mode_ext(struct mt76_phy *phy, struct ieee80211_vif *vif,
			 ...
			 bool enable)
{
	...
	basic_req->phymode_ext = mt7902_get_phy_mode_ext(phy, vif, band, sta);
...
static void
mt7902_mcu_bss_qos_tlv(struct sk_buff *skb, struct ieee80211_vif *vif)
{
...
static void
mt7902_mcu_bss_color_tlv(struct sk_buff *skb, struct ieee80211_vif *vif,
			 bool enable)
{
...
static void
mt7902_mcu_bss_ifs_tlv(struct sk_buff *skb, struct ieee80211_vif *vif)
{
	struct mt792x_vif *mvif = (struct mt792x_vif *)vif->drv_priv;
...
int mt7902_mcu_set_timing(struct mt792x_phy *phy,
			  struct ieee80211_vif *vif)
{
	struct mt792x_vif *mvif = (struct mt792x_vif *)vif->drv_priv;
	struct mt792x_dev *dev = phy->dev;
	...
	mt7902_mcu_bss_ifs_tlv(skb, vif);
			    ...
			    int enable)
{
	...
	mt7902_mcu_bss_basic_tlv(skb, vif, sta, ctx, phy->mt76,
				 mvif->sta.wcid.idx, enable);
	mt7902_mcu_bss_sec_tlv(skb, vif);

	mt7902_mcu_bss_bmc_tlv(skb, phy, ctx, vif, sta);
	mt7902_mcu_bss_qos_tlv(skb, vif);
	mt7902_mcu_bss_mld_tlv(skb, vif, sta);
	mt7902_mcu_bss_ifs_tlv(skb, vif);

	if (vif->bss_conf.he_support) {
		mt7902_mcu_bss_he_tlv(skb, vif, phy);
		mt7902_mcu_bss_color_tlv(skb, vif, enable);
	}
	...
	return mt7902_mcu_set_chctx(phy->mt76, &mvif->mt76, ctx);
}

int mt7902_mcu_set_dbdc(struct mt76_phy *phy)
{
	struct mt76_dev *mdev = phy->dev;
...
int mt7902_mcu_hw_scan(struct mt76_phy *phy, struct ieee80211_vif *vif,
...
EXPORT_SYMBOL_GPL(mt7902_mcu_hw_scan);

int mt7902_mcu_sched_scan_req(struct mt76_phy *phy,
			      struct ieee80211_vif *vif,
...
EXPORT_SYMBOL_GPL(mt7902_mcu_sched_scan_req);

int
mt7902_mcu_sched_scan_enable(struct mt76_phy *phy,
			     struct ieee80211_vif *vif,
			     bool enable)
{
	struct mt76_dev *mdev = phy->dev;
...
int mt7902_mcu_cancel_hw_scan(struct mt76_phy *phy,
			      struct ieee80211_vif *vif)
{
	struct mt76_vif *mvif = (struct mt76_vif *)vif->drv_priv;
	struct {
...
EXPORT_SYMBOL_GPL(mt7902_mcu_cancel_hw_scan);
...
static int
__mt7902_mcu_set_clc(struct mt792x_dev *dev, u8 *alpha2,
		     enum environment_cap env_cap,
		     struct mt7902_clc *clc, u8 idx)
{
	struct mt7902_clc_segment *seg;
	struct sk_buff *skb;
	struct {
		u8 rsv[4];
		__le16 tag;
	...
	for (i = 0; i < clc->nr_country; i++) {
		...
		seg = (struct mt7902_clc_segment *)clc->data
			  + rule->seg_idx - 1;
...
int mt7902_mcu_fill_message(struct mt76_dev *mdev, struct sk_buff *skb,
...
EXPORT_SYMBOL_GPL(mt7902_mcu_fill_message);
...
int mt7902_mcu_set_radio_en(struct mt792x_phy *phy, bool enable)
{
	struct {
		u8 band_idx;
		u8 _rsv[3];
...
static void
mt7902_mcu_build_sku(struct mt76_dev *dev, s8 *sku,
...
static int
mt7902_mcu_rate_txpower_band(struct mt76_phy *phy,
			     enum nl80211_band band)
{
	...
	struct mt76_power_limits *limits;
	struct mt7902_sku_tlv *sku_tlbv;
	const u8 *ch_list;
	...
	for (i = 0; i < batch_size; i++) {
		struct mt7902_tx_power_limit_tlv *tx_power_tlv;
		...
		tx_power_tlv = (struct mt7902_tx_power_limit_tlv *)
		...
		for (j = 0; j < num_ch; j++, idx++) {
			...
			mt7902_mcu_build_sku(dev, sku_tlbv->pwr_limit,
					     limits, band);
...
int mt7902_mcu_set_rate_txpower(struct mt76_phy *phy)
{
	int err;

	if (phy->cap.has_2ghz) {
		err = mt7902_mcu_rate_txpower_band(phy,
						   NL80211_BAND_2GHZ);
		if (err < 0)
			return err;
	}
