...
static void mt7902_mac_sta_poll(struct mt792x_dev *dev)
{
	...
	while (true) {
		...
		idx = msta->wcid.idx;
		addr = mt7902_mac_wtbl_lmac_addr(dev, idx, MT_WTBL_AC0_CTT_OFFSET);
		...
		if (rate->flags & RATE_INFO_FLAGS_EHT_MCS) {
			addr = mt7902_mac_wtbl_lmac_addr(dev, idx, 5);
			val = mt76_rr(dev, addr);
		...
		addr = mt7902_mac_wtbl_lmac_addr(dev, idx, 34);
		val = mt76_rr(dev, addr);
...
void mt7902_mac_set_fixed_rate_table(struct mt792x_dev *dev,
...
static int
mt7902_mac_fill_rx_rate(struct mt792x_dev *dev,
...
static void
mt7902_mac_write_txwi_8023(__le32 *txwi, struct sk_buff *skb,
...
static void
mt7902_mac_write_txwi_80211(struct mt76_dev *dev, __le32 *txwi,
			    struct sk_buff *skb,
...
void
mt7902_mac_write_txwi(struct mt76_dev *dev, __le32 *txwi,
		      ...
		      enum mt76_txq_id qid, u32 changed)
{
	...
	if (is_8023)
		mt7902_mac_write_txwi_8023(txwi, skb, wcid);
	else
		mt7902_mac_write_txwi_80211(dev, txwi, skb, key);
...
EXPORT_SYMBOL_GPL(mt7902_mac_write_txwi);

static void mt7902_tx_check_aggr(struct ieee80211_sta *sta, __le32 *txwi)
{
	struct mt792x_sta *msta;
	u16 fc, tid;
	u32 val;
...
static bool
mt7902_mac_add_txs_skb(struct mt792x_dev *dev, struct mt76_wcid *wcid,
...
void mt7902_mac_add_txs(struct mt792x_dev *dev, void *data)
{
	...
	mt7902_mac_add_txs_skb(dev, wcid, pid, txs_data);
	if (!wcid->sta)
		goto out;
...
void mt7902_txwi_free(struct mt792x_dev *dev, struct mt76_txwi_cache *t,
...
EXPORT_SYMBOL_GPL(mt7902_txwi_free);

static void
mt7902_mac_tx_free(struct mt792x_dev *dev, void *data, int len)
{
	...
	for (cur_info = &tx_free[2]; count < total; cur_info++) {
		...
		for (i = 0; i < 2; i++) {
			...
			mt7902_txwi_free(dev, txwi, sta, 0, &free_list);
		}
	}
...
void mt7902_coredump_work(struct work_struct *work)
{
	...
	while (true) {
		...
		skb_pull(skb, sizeof(struct mt7902_mcu_rxd) + 8);
			   ...
			   struct sk_buff *skb)
{
	...
	mt7902_mac_write_txwi(&dev->mt76, txwi, skb, wcid, key, pid, qid, 0);
