...
struct mt7902_mcu_scan_chinfo_event {
	u8 nr_chan;
	u8 alpha2[3];
} __packed;
...
#define MT7902_STA_UPDATE_MAX_SIZE	(sizeof(struct sta_req_hdr) +		\
...
struct mt7902_sku_tlv {
	u8 channel;
	s8 pwr_limit[MT_CONNAC3_SKU_POWER_LIMIT];
} __packed;

struct mt7902_tx_power_limit_tlv {
	u8 rsv[4];
...
mt7902_mcu_get_cipher(int cipher)
{
	switch (cipher) {
...
int mt7902_mcu_sched_scan_enable(struct mt76_phy *phy,
				 struct ieee80211_vif *vif,
				 bool enable);
