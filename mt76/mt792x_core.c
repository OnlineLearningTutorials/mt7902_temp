...
int mt792x_init_wiphy(struct ieee80211_hw *hw)
{
	...
	printk(KERN_INFO "mt792x_core.c - mt792x_init_wiphy - dev->pm.enable:%d", dev->pm.enable);
	if (dev->pm.enable)
		ieee80211_hw_set(hw, CONNECTION_MONITOR);
...
{
	int ret;
	printk(KERN_INFO "mt792x_core.c - mt792x_load_firmware - chip:0x%x mt792x_patch_name : %s", mt76_chip(&dev->mt76), mt792x_patch_name(dev));
