...
int mt7902_dma_init(struct mt7902_dev *dev, struct mt7902_phy *phy2)
{
	...
	if (mtk_wed_device_active(&mdev->mmio.wed)) {
		if (!is_mt798x(mdev)) {
			u8 wed_control_rx1 = is_mt7992(mdev) ? 1 : 2;
	...
	if (phy2) {
		ret = mt7902_init_tx_queues(phy2,
	...
	/* event from WA */
	if (mtk_wed_device_active(&mdev->mmio.wed) && is_mt7992(mdev)) {
...
int mt7902_dma_reset(struct mt7902_dev *dev, bool force)
{
	...
	if (mtk_wed_device_active(wed) && is_mt7992(&dev->mt76))
		mt76_rmw(dev, MT_WFDMA0_EXT0_CFG, MT_WFDMA0_EXT0_RXWB_KEEP,
			 MT_WFDMA0_EXT0_RXWB_KEEP);
