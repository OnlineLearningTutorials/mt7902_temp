...
static int
mt7902_tm_set_tx_power(struct mt7902_phy *phy)
{
...
static int
mt7902_tm_set_freq_offset(struct mt7902_phy *phy, bool en, u32 val)
{
	struct mt7902_dev *dev = phy->dev;
	struct mt7902_tm_cmd req = {
		.testmode_en = en,
...
static int
mt7902_tm_mode_ctrl(struct mt7902_dev *dev, bool enable)
{
	struct {
		u8 format_id;
		bool enable;
...
static int
mt7902_tm_set_trx(struct mt7902_phy *phy, int type, bool en)
{
	struct mt7902_dev *dev = phy->dev;
	struct mt7902_tm_cmd req = {
		.testmode_en = 1,
...
static int
mt7902_tm_clean_hwq(struct mt7902_phy *phy, u8 wcid)
{
	struct mt7902_dev *dev = phy->dev;
	struct mt7902_tm_cmd req = {
		.testmode_en = 1,
...
static int
mt7902_tm_set_slot_time(struct mt7902_phy *phy, u8 slot_time, u8 sifs)
{
	struct mt7902_dev *dev = phy->dev;
	struct mt7902_tm_cmd req = {
...
static int
mt7902_tm_set_tam_arb(struct mt7902_phy *phy, bool enable, bool mu)
{
	struct mt7902_dev *dev = phy->dev;
	u32 op_mode;
	...
	return mt7902_mcu_set_muru_ctrl(dev, MURU_SET_ARB_OP_MODE, op_mode);
}

static int
mt7902_tm_set_wmm_qid(struct mt7902_phy *phy, u8 qid, u8 aifs, u8 cw_min,
		      u16 cw_max, u16 txop)
{
	struct mt7902_vif *mvif = (struct mt7902_vif *)phy->monitor_vif->drv_priv;
	struct mt7902_mcu_tx req = { .total = 1 };
	...
	return mt7902_mcu_update_edca(phy->dev, &req);
}

static int
mt7902_tm_set_ipg_params(struct mt7902_phy *phy, u32 ipg, u8 mode)
{
...
#define BBP_PROC_TIME	1500
	...
	mt7902_tm_set_slot_time(phy, slot_time, sifs);

	return mt7902_tm_set_wmm_qid(phy,
				     mt76_connac_lmac_mapping(IEEE80211_AC_BE),
				     aifsn, cw, cw, 0);
}

static int
mt7902_tm_set_tx_len(struct mt7902_phy *phy, u32 tx_time)
{
	struct mt76_phy *mphy = phy->mt76;
...
static void
mt7902_tm_reg_backup_restore(struct mt7902_phy *phy)
{
...
static void
mt7902_tm_init(struct mt7902_phy *phy, bool en)
{
	struct mt7902_dev *dev = phy->dev;
	...
	mt7902_mcu_set_sku_en(phy, !en);

	mt7902_tm_mode_ctrl(dev, en);
	mt7902_tm_reg_backup_restore(phy);
	mt7902_tm_set_trx(phy, TM_MAC_TXRX, !en);

	mt7902_mcu_add_bss_info(phy, phy->monitor_vif, en);
	mt7902_mcu_add_sta(dev, phy->monitor_vif, NULL, en);

	if (!en)
		mt7902_tm_set_tam_arb(phy, en, 0);
}

static void
mt7902_tm_update_channel(struct mt7902_phy *phy)
{
	mutex_unlock(&phy->dev->mt76.mutex);
	mt7902_set_channel(phy);
	mutex_lock(&phy->dev->mt76.mutex);

	mt7902_mcu_set_chan_info(phy, MCU_EXT_CMD(SET_RX_PATH));
}

static void
mt7902_tm_set_tx_frames(struct mt7902_phy *phy, bool en)
{
	...
	mt7902_tm_set_trx(phy, TM_MAC_RX_RXV, false);
	mt7902_tm_clean_hwq(phy, dev->mt76.global_wcid.idx);

	if (en) {
		mt7902_tm_update_channel(phy);
	...
	mt7902_tm_set_ipg_params(phy, ipg, td->tx_rate_mode);
	mt7902_tm_set_tx_len(phy, tx_time);
	...
	mt7902_tm_set_trx(phy, TM_MAC_TX, en);
}

static void
mt7902_tm_set_rx_frames(struct mt7902_phy *phy, bool en)
{
	mt7902_tm_set_trx(phy, TM_MAC_RX_RXV, false);
	...
	if (en) {
		...
		mt7902_tm_set_trx(phy, TM_MAC_RX_RXV, en);
	}
}

static int
mt7902_tm_rf_switch_mode(struct mt7902_dev *dev, u32 oper)
{
	struct mt7902_tm_rf_test req = {
...
static int
mt7902_tm_set_tx_cont(struct mt7902_phy *phy, bool en)
{
...
out:
	if (!en) {
		...
		return mt7902_tm_rf_switch_mode(dev, RF_OPER_NORMAL);
	}

	mt7902_tm_rf_switch_mode(dev, RF_OPER_RF_TEST);
	mt7902_tm_update_channel(phy);
...
static void
mt7902_tm_update_params(struct mt7902_phy *phy, u32 changed)
{
	struct mt76_testmode_data *td = &phy->mt76->test;
	bool en = phy->mt76->test.state != MT76_TM_STATE_OFF;

	if (changed & BIT(TM_CHANGED_FREQ_OFFSET))
		mt7902_tm_set_freq_offset(phy, en, en ? td->freq_offset : 0);
	if (changed & BIT(TM_CHANGED_TXPOWER))
		mt7902_tm_set_tx_power(phy);
}

static int
mt7902_tm_set_state(struct mt76_phy *mphy, enum mt76_testmode_state state)
{
	struct mt76_testmode_data *td = &mphy->test;
	...
	if (prev_state == MT76_TM_STATE_TX_FRAMES ||
		...
		mt7902_tm_set_tx_frames(phy, state == MT76_TM_STATE_TX_FRAMES);
	else if (prev_state == MT76_TM_STATE_RX_FRAMES ||
		 state == MT76_TM_STATE_RX_FRAMES)
		mt7902_tm_set_rx_frames(phy, state == MT76_TM_STATE_RX_FRAMES);
	else if (prev_state == MT76_TM_STATE_TX_CONT ||
		 state == MT76_TM_STATE_TX_CONT)
		mt7902_tm_set_tx_cont(phy, state == MT76_TM_STATE_TX_CONT);
	else if (prev_state == MT76_TM_STATE_OFF ||
		 state == MT76_TM_STATE_OFF)
		mt7902_tm_init(phy, !(state == MT76_TM_STATE_OFF));
	...
	if ((state == MT76_TM_STATE_IDLE &&
		...
		mt7902_tm_update_params(phy, changed);
	}
...
static int
mt7902_tm_set_params(struct mt76_phy *mphy, struct nlattr **tb,
...
static int
mt7902_tm_dump_stats(struct mt76_phy *mphy, struct sk_buff *msg)
{
	struct mt7902_phy *phy = mphy->priv;
	struct mt7902_dev *dev = phy->dev;
	enum mt76_rxq_id q;
	...
	fcs_err = is_mt7902(&dev->mt76) ? FIELD_GET(MT_MIB_SDR3_FCS_ERR_MASK, cnt) :
...
const struct mt76_testmode_ops mt7902_testmode_ops = {
	.set_state = mt7902_tm_set_state,
	.set_params = mt7902_tm_set_params,
	.dump_stats = mt7902_tm_dump_stats,
};
