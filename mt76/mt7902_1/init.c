...
static const struct ieee80211_iface_combination if_comb[] = {
	{
		...
		.max_interfaces = MT7902_MAX_INTERFACES,
...
static ssize_t mt7902_thermal_temp_store(struct device *dev,
					 struct device_attribute *attr,
...
static SENSOR_DEVICE_ATTR_RW(temp1_crit, mt7902_thermal_temp, 1);
static SENSOR_DEVICE_ATTR_RW(temp1_max, mt7902_thermal_temp, 2);
static SENSOR_DEVICE_ATTR_RO(throttle1, mt7902_thermal_temp, 3);
...
static const struct thermal_cooling_device_ops mt7902_thermal_ops = {
	.get_max_state = mt7902_thermal_get_max_throttle_state,
	.get_cur_state = mt7902_thermal_get_cur_throttle_state,
	.set_cur_state = mt7902_thermal_set_cur_throttle_state,
};
...
#define TO_HW_TICK(_t)	(((_t) > HW_TICK) ? ((_t) / HW_TICK) : HW_TICK)
	...
	mt7902_led_set_config(led_cdev, delta_on, delta_off);
				  ...
				  struct ieee80211_supported_band *sband)
{
	...
	for (i = 0; i < sband->n_channels; i++) {
		...
		for (j = 0; j < n_chains; j++) {
			...
			val = mt7902_eeprom_get_target_power(dev, chan, j);
...
mt7902_init_wiphy(struct mt7902_phy *phy)
{
	...
	hw->sta_data_size = sizeof(struct mt7902_sta);
	hw->vif_data_size = sizeof(struct mt7902_vif);
	...
	if (phy->mt76->cap.has_5ghz) {
		...
		if (!is_mt7992(&dev->mt76) || !dev->dbdc_support)
...
{
	int i;
	u32 rx_len = is_mt7992(&dev->mt76) ? 0x400 : 0x680;
...
int mt7902_txbf_init(struct mt7902_dev *dev)
{
	...
	if (dev->dbdc_support) {
		ret = mt7902_mcu_set_txbf(dev, MT_BF_MODULE_UPDATE);
		if (ret)
			return ret;
	}

	/* trigger sounding packets */
	ret = mt7902_mcu_set_txbf(dev, MT_BF_SOUNDING_ON);
	if (ret)
		return ret;
...
mt7902_init_hardware(struct mt7902_dev *dev, struct mt7902_phy *phy2)
{
	...
	ret = mt7902_mcu_init(dev);
	if (ret)
		return ret;

	ret = mt7902_eeprom_init(dev);
	if (ret < 0)
		return ret;
