...
static ssize_t mt7921_thermal_temp_show(struct device *dev,
					...
					char *buf)
{
	printk(KERN_DEBUG "init.c - mt7921_thermal_temp_show(dev, attr, buf)");
	...
	case 0: {
		...
		printk(KERN_DEBUG "init.c - mt7921_thermal_temp_show(dev, attr, buf) - temp: %u", temperature);
...
{
	printk(KERN_DEBUG "init.c - mt7921_thermal_init(phy)");
	...
	if (!name) {
		...
		printk(KERN_DEBUG "init.c - mt7921_thermal_init(phy) - %s", name);
...
{
	printk(KERN_DEBUG "init.c - mt7921_regd_channel_update(wiphy, dev)");
...
{
	printk(KERN_DEBUG "init.c - mt7921_regd_update(dev)");
	struct mt76_dev *mdev = &dev->mt76;
...
{
	printk(KERN_DEBUG "init.c - mt7921_regd_notifier(wiphy, request)");
...
{
	printk(KERN_DEBUG "init.c - mt7921_mac_init(dev)");
	int i, ret;
...
{
	printk(KERN_DEBUG "init.c - __mt7921_init_hardware(dev)");
	int ret;
...
out:
	if (ret == 0)
		dev_info(dev->mt76.dev, "__mt7921_init_hardware sequence completed successfully\n");
...
{
	printk(KERN_DEBUG "init.c - mt7921_init_hardware(dev)");
	int ret, i;
...
{
	printk(KERN_DEBUG "init.c - mt7921_init_work(work)");
	...
	if (ret) {
		dev_err(dev->mt76.dev, "mt7921_init_hardware failed: %d\n", ret);
		return;
	}
...
{
	printk(KERN_DEBUG "init.c - mt7921_register_device(dev)");
	...
	dev->mphy.hw->wiphy->available_antennas_tx = dev->mphy.chainmask;
    	...
    	dev_info(dev->mt76.dev, "Scheduling mt7921_init_work\n");
