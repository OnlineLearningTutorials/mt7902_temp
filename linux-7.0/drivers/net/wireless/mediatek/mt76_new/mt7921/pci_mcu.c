...
{
	printk(KERN_DEBUG "pci_mcu.c - mt7921e_driver_own(dev)");
...
mt7921_mcu_send_message(struct mt76_dev *mdev, struct sk_buff *skb,
			int cmd, int *seq)
{
	//printk(KERN_DEBUG "pci_mcu.c - mt7921_mcu_send_message(mdev, skb, cmd: %d, seq: %d)", cmd, seq);
...
{
	printk(KERN_DEBUG "pci_mcu.c - mt7921e_mcu_init(dev)");
